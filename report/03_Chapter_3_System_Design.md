...
| Model Name | Primary Keys / Relations | Description |
|------------|-------------------------|-------------|
| **User** | `id` (UUID) | Stores user profile information (name, email, avatar). |
| **Account** | `provider`, `providerAccountId` | Managed by Better Auth, handling OAuth tokens and sessions. |
| **Repository** | `id`, Relation to `User` | Stores tracked GitHub repos (`name`, `owner`, `githubId`). |
| **Review** | `id`, Relation to `Repository`| Tracks `prNumber`, markdown `review` text, and job `status`. |
