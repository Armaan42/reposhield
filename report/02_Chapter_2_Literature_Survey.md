...
1.  **Lack of Cross-File Contextual Awareness:** When a PR is submitted, standard automated tools only analyze the *diff* (the lines changed). They fail to retrieve or understand how a change in `utils.ts` might inadvertently break a database query in `actions.ts`. Consequently, this isolated analysis fails to detect downstream architectural regressions and systemic bugs across disconnected code modules.
2.  **Serverless Execution Limits:** Modern web applications frequently use serverless functions (e.g., AWS Lambda, Vercel). These platforms impose strict execution timeouts (often 10 seconds). Passing a large repository context to an LLM and waiting for a comprehensive review generation routinely exceeds these timeouts, causing the webhook to fail and drop the PR event entirely. This mismatch between rigid serverless compute limits and long-running generative inference prevents direct synchronous integration of AI agents within standard webhook pipelines.
3.  **High Cost and Latency of Large Context Windows:** While some modern LLMs boast massive context windows (e.g., 1 million tokens), blindly feeding an entire repository into the prompt for every PR review is prohibitively expensive and introduces severe latency, making it unsuitable for rapid CI/CD cycles. Moreover, flooding the LLM with irrelevant codebase files increases the prompt noise, which dramatically degrades the model's focus and leads to less precise review feedback.
